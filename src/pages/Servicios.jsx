    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{servicios.length} artículo{servicios.length !== 1 ? "s" : ""}</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
          <h3 className="font-bold text-slate-800">Nuevo artículo</h3>
            ...
            {itemsMaterial.map((item, idx) => (
              <div key={idx} className="flex gap-2 mb-2 items-start">
                <div className="flex-1 space-y-2">
                  <select className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
      ...
      {servicios.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="font-semibold text-slate-600">Sin artículos registrados</p>
          <p className="text-sm text-slate-400 mt-1">Creá tu primer artículo del catálogo.</p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          {servicios.map((s, idx) => {
