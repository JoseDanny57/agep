    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{materiales.length} materiales</p>
        </div>
      ...
      {bajosStock.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3">
          <p className="text-xs font-bold text-amber-700">⚠️ {bajosStock.length} material(es) con stock bajo</p>
        </div>
      )}

      {showForm && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
          <h3 className="font-bold text-slate-800">{editando ? "Editar material" : "Nuevo material"}</h3>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5">Costo/unidad</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5">Stock actual</label>
              <input type="number" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="0" value={form.stock_actual}
                onChange={e => setForm(f => ({ ...f, stock_actual: e.target.value }))} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5">Stock mínimo</label>
              <input type="number" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="font-semibold text-slate-600">Sin materiales registrados</p>
          <p className="text-sm text-slate-400 mt-1">Agrega tus materiales e insumos.</p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          {materiales.map((mat, idx) => {
            ...
            return (
              <div key={mat.id}
                className={`flex items-center gap-3 px-4 py-3 ${idx < materiales.length - 1 ? "border-b border-slate-50" : ""}`}>
                <div className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${bajo ? "bg-amber-400" : "bg-green-400"}`} />
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-slate-800 text-sm truncate">{mat.nombre}</p>
                  <p className="text-xs text-slate-400">
                    {mat.stock_actual} {mat.unidad}
                    {bajo && <span className="ml-1 text-amber-500">⚠️ bajo</span>}
                    {mat.costo_unitario > 0 && <span className="ml-2">{fmtCosto(mat.costo_unitario)}/{mat.unidad.slice(0, 3)}</span>}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => abrirEditar(mat)} className="text-slate-400 hover:text-blue-500 text-xs px-2 py-1 rounded-lg hover:bg-blue-50">✏️</button>
                  <button onClick={() => eliminar(mat.id)} className="text-slate-300 hover:text-red-400 text-xs">✕</button>
                </div>
              </div>
            );
          })}
